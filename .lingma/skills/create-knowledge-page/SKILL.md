...
**除非用户明确要求确认，否则 Skill 应自动执行所有步骤，无需等待用户回复 "Y"。**

- ✅ **自动执行**：检测到元数据存在 → 直接生成页面 → 自动部署
- ✅ **自动执行**：批量创建模式 → 逐个创建 → 统一部署
- ✅ **自动执行**：标准流程 → 更新 chapters.ts → 创建页面 → 验证 → 部署
- ❌ **仅在以下情况需要确认**：
  - 文件已存在且用户未明确说"覆盖"
  - 检测到严重冲突或数据不一致
  - 用户明确要求"先让我看看"或"需要确认"

**默认行为：快速、自动化、无中断地完成整个流程。**
...
5. **逐个创建页面**（可并行执行）
   ...
   - 全部完成后，汇总报告
   - 💡 **性能优化**：如果多个页面独立，可使用 `dispatching-parallel-agents` skill 同时创建
...
**💡 智能推断技巧：**
- 如果用户提到 "HashMap" → 自动推断属于 "02-collections" 章节
- 如果用户提到 "多线程" → 自动推断属于 "03-concurrency" 章节
- 如果用户提到 "Python" → 自动推断属于 "04-python" 章节
- 根据知识点复杂度自动推荐难度：基础概念 → Junior，框架使用 → Senior，源码分析 → Expert
...
- 在浏览器中预览页面
- **⚡ 自动化要求**：如果构建失败，自动分析错误日志并修复常见问题（如导入路径错误、类型不匹配等）
...
4. **完成提示**：
   - 输出成功消息和访问 URL
   - 如果有错误，自动重试一次并报告详细错误信息

**💡 性能优化建议：**
- 批量创建多个页面时，只在最后统一执行一次部署，避免重复构建
- 如果部署失败，自动检查 SSH 连接、Docker 状态、磁盘空间等常见问题
...
- **⚡ 自动化原则**：除非用户明确要求确认，否则自动执行所有步骤，不等待 "Y/n" 回复
...
```typescript
// 当需要同时创建多个独立页面时
const pages = ['set-deep-dive', 'map-framework', 'hashmap-deep-dive']

// 可以并行创建，提升效率
pages.forEach(slug => {
  // 为每个页面启动独立的 agent
  // 注意：确保每个 agent 的操作是独立的，不会互相干扰
})
```

**注意事项：**
- 仅在页面之间无依赖关系时使用并行
- 所有章节的元数据必须先在 `chapters.ts` 中定义完成
- 部署操作必须在所有页面创建完成后统一执行
...
| 问题 | 自动修复策略 |
|------|------------|
| TypeScript 类型错误 | 检查导入路径，修正类型定义 |
| 组件未找到 | 扫描组件目录，修正导入语句 |
| 构建失败 | 分析错误日志，修复语法错误 |
| 部署失败 | 检查 SSH 连接、Docker 状态、磁盘空间 |
| slug 与文件名不一致 | 自动重命名文件以匹配 slug |
...
📝 正在创建知识点页面...
✅ 步骤 1/8: 检测元数据
✅ 步骤 2/8: 更新 chapters.ts
🔄 步骤 3/8: 生成页面内容 (预计 2-3 分钟)
✅ 步骤 4/8: 验证构建
✅ 步骤 5/8: 质量检查通过
🚀 步骤 6/8: 提交到 Git
🚀 步骤 7/8: 部署到远程服务器
...
✨ 完成！访问: https://your-domain.com/docs/02-collections/concurrent-hashmap
```
...
错误信息: Cannot find module '../../components/knowledge/XxxAnimator'

可能原因:
1. 组件文件不存在
2. 导入路径错误

自动修复: 已移除对 XxxAnimator 的引用，使用标准组件替代
...
5. ✅ 步骤 8：自动提交 Git → 调用 remote-deploy → 部署到远程服务器
6. ✅ 完成提示："✅ ConcurrentHashMap 页面已创建并部署成功！访问 https://your-domain.com/docs/02-collections/concurrent-hashmap 查看"

---

### 场景 2：元数据不存在，标准流程（全自动）
...
**Skill 执行流程（自动检测并批量创建）：**
1. ✅ 步骤 0（批量模式）：扫描 `02-collections` 章节下所有文章
2. 🔍 检测哪些页面文件缺失
3. 📋 展示待创建列表
4. 🔄 逐个创建缺失的页面（快速模式）
   - 💡 **性能优化**：使用 `dispatching-parallel-agents` skill 同时创建多个页面（如 3 个页面并行）
5. ✅ 全部完成后统一部署
6. ✅ 完成提示："✅ 02-collections 章节的 3 个页面已全部创建并部署成功！"
...
4. 💡 **自动修复**：如果用户未明确说"只检查"，则自动修复发现的问题
5. ✅ 修复后重新验证，确保所有问题已解决
...
**用户输入：**
```
更新 HashMap 页面的面试真题部分
```

**Skill 执行流程：**
1. ✅ 定位到 `site/src/pages/articles/hashmap-deep-dive.tsx`
2. 🔍 分析现有内容，识别需要更新的部分
3. ✍️ 补充或更新面试真题（确保至少 5 题）
4. ✅ 验证构建通过
5. 🚀 自动部署更新
...
### 核心原则
- ⚡ **自动化优先**：无需确认，自动执行
- 📝 **先写元数据**：chapters.ts → 页面文件
- 🔍 **文件名 = slug**：严格一致
- 🧩 **使用封装组件**：禁止内联拼凑
- ✅ **12 项要素齐全**：定义、架构、原理、源码、流程、实验场、边注、上下文、误区、面试、对比、关联

### 常见命令
```bash
# 创建单个页面
创建 ConcurrentHashMap 知识点
...
### 关键文件
- 📄 元数据：`site/src/data/chapters.ts`
- 📄 页面：`site/src/pages/articles/<slug>.tsx`
- 📄 组件：`site/src/components/knowledge/` 和 `site/src/components/ui/`
- 📄 类型：`site/src/data/types.ts`

### 常用组件
- `KnowledgeLayout` - 页面布局容器
- `SmartTOC` - 侧边栏目录（直接渲染，不用 aside 包裹）
- `ArticleNav` - 文章导航（上一篇/下一篇）
- `Playground` - 代码实验场
- `InteractiveFlow` - 交互式流程图
- `ContextSwitcher` - 上下文切换
- `SideNote` - 边注/扩展
- `Callout` - 提示框（info/warning/danger）
- `DiagramBlock` - 架构图
- `InterviewSection` - 面试真题
