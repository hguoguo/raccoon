...
- 服务器上已安装 Docker
- 服务器上通过 nvm 安装了 Node.js 和 npm（需要手动加载 nvm 环境）
...
# Load nvm environment (SSH doesn't load .bashrc automatically)
...
```bash
git push && ssh xb-nas 'export NVM_DIR="$HOME/.nvm" && [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh" && cd /vol1/1000/java && if [ -d raccoon-edu ]; then cd raccoon-edu && git pull; else git clone https://gitee.com/huaguoguo/raccoon-edu.git && cd raccoon-edu; fi && cd site && npm install && npm run build && cd .. && docker build -t raccoon-edu -f docker/Dockerfile . && docker stop raccoon-edu || true && docker rm raccoon-edu || true && docker run -d --name raccoon-edu -p 5300:5300 --restart unless-stopped raccoon-edu'
```
...
7. **nvm 环境加载**：SSH 远程执行不会自动加载 `.bashrc`，必须在每次使用 npm 前手动加载 nvm 环境
...
# Load nvm environment (critical for npm commands)
export NVM_DIR="$HOME/.nvm"
[ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"
