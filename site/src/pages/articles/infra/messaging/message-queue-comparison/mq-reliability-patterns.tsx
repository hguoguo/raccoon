...
const tocItems: TocItem[] = [
  ...
  { id: 'transaction-message', text: '四、事务消息', level: 2 },
...
export default function MqReliabilityPatterns({ meta }: { meta: KnowledgeNode }) {
  return (
        ...
        <KnowledgeLayout meta={meta}>
          ...
          <blockquote className="border-l-[3px] border-accent pl-4 sm:pl-5 py-2 my-5 bg-accent-soft/40 rounded-r-paper-md">
            <p className="text-[15px] sm:text-base text-ink-light leading-[1.8] font-medium">
              消息可靠性保障是通过<strong className="text-accent">ACK 机制、幂等消费、事务消息、死信队列、重试策略</strong>等技术手段，确保消息在分布式系统中不丢失、不重复、不错序地传输和处理。
            </p>
          </blockquote>

          <Callout type="tip" title="为什么需要可靠性保障？">
            分布式系统面临网络故障、节点宕机、磁盘损坏等风险，消息可能在生产、存储、消费的任何环节丢失或重复。可靠性保障机制通过多层防护，将消息丢失概率降至极低（如 99.999%），满足金融、电商等关键业务的需求。
          </Callout>
          ...
          <p className="text-[14px] sm:text-[15px] leading-[1.8] sm:leading-[1.9] text-ink-muted mb-4">
            消息可靠性涉及生产者、Broker、消费者三个环节，需要端到端的保障。
          </p>

          <DiagramBlock title="消息可靠性保障体系">
...
目标：在保证可靠性的前提下，平衡性能和复杂度
          ...
          <SideNote label="CAP 权衡">
            消息队列的可靠性设计本质上是<strong>CAP 定理</strong>的权衡：① 强一致性（CP）：同步复制，性能低但数据不丢；② 最终一致性（AP）：异步复制，性能高但可能短暂不一致。大多数 MQ 采用折中方案：默认异步复制，关键业务可配置同步复制。
          </SideNote>
          ...
          <p className="text-[14px] sm:text-[15px] leading-[1.8] sm:leading-[1.9] text-ink-muted mb-4">
            ACK（Acknowledgment）是消息可靠性的基础，确保消息被正确接收和处理。
          </p>
...
# - Broker：关键业务使用同步复制，非关键使用异步复制`}
            language="python"
            highlights={[7, 14, 21, 28, 35, 42, 50]}
            filename="ack-mechanism.py"
            description="ACK 确认机制"
          />

          <Callout type="danger" title="自动 ACK 的风险">
            自动 ACK 模式下，消息一旦投递给消费者就立即确认，如果消费者处理失败或宕机，消息将永久丢失。<strong>生产环境必须使用手动 ACK</strong>，确保业务逻辑执行成功后才确认消息。虽然增加了代码复杂度，但这是可靠性的必要代价。
          </Callout>
          ...
          <p className="text-[14px] sm:text-[15px] leading-[1.8] sm:leading-[1.9] text-ink-muted mb-4">
            幂等性确保同一条消息被多次消费时，结果一致，避免重复处理导致的数据错误。
          </p>
...
# - 通用场景：Token 机制`}
            language="python"
            highlights={[6, 23, 40, 57, 74, 90]}
            filename="idempotency.py"
            description="幂等性实现方案"
          />

          <SideNote label="幂等性的重要性">
            在 At Least Once 语义下，消息可能重复投递。如果消费者不是幂等的，会导致：<strong>重复扣款、重复发货、数据重复统计</strong>等严重问题。因此，<strong>所有消费者都应该是幂等的</strong>，无论 MQ 是否保证 Exactly-Once。
          </SideNote>

          <h2 id="transaction-message" className="font-display font-bold text-[20px] sm:text-display-md tracking-tight mt-8 sm:mt-12 mb-3 sm:mb-4 pb-[10px] border-b border-border-light text-ink">
            四、事务消息
          </h2>
          <p className="text-[14px] sm:text-[15px] leading-[1.8] sm:leading-[1.9] text-ink-muted mb-4">
            事务消息保证消息发送和本地事务的原子性，要么都成功，要么都失败。
          </p>
...
# - 本地事务执行时间不宜过长（避免长时间持有锁）`}
            language="python"
            highlights={[6, 38, 46, 56, 68, 80, 92]}
            filename="transaction-message.py"
            description="事务消息实现"
          />

          <Callout type="warning" title="事务消息的性能代价">
            事务消息需要额外的往返和回查机制，吞吐量比普通消息低 30-50%。仅在<strong>确实需要强一致性</strong>的场景使用，如金融交易、订单处理。对于日志收集、指标上报等场景，使用普通消息 + 幂等消费即可。
          </Callout>

          <h2 id="dead-letter-queue" className="font-display font-bold text-[20px] sm:text-display-md tracking-tight mt-8 sm:mt-12 mb-3 sm:mb-4 pb-[10px] border-b border-border-light text-ink">
            五、死信队列
          </h2>
          <p className="text-[14px] sm:text-[15px] leading-[1.8] sm:leading-[1.9] text-ink-muted mb-4">
            死信队列（DLQ）存储多次重试仍失败的消息，避免阻塞正常消息的处理。
          </p>
...
# - 建立处理流程（人工审核、自动重试、丢弃）`}
            language="python"
            highlights={[6, 15, 32, 45, 64, 80, 95]}
            filename="dead-letter-queue.py"
            description="死信队列实现"
          />

          <SideNote label="死信处理策略">
            死信队列不是终点，而是<strong>问题诊断的起点</strong>。应建立完善的死信处理流程：① 实时监控死信数量；② 自动告警；③ 分类处理（代码 Bug → 修复重发，数据异常 → 人工修正，依赖故障 → 等待恢复）；④ 定期复盘，优化系统。
          </SideNote>

          <h2 id="retry-strategy" className="font-display font-bold text-[20px] sm:text-display-md tracking-tight mt-8 sm:mt-12 mb-3 sm:mb-4 pb-[10px] border-b border-border-light text-ink">
            六、重试策略
          </h2>
          <p className="text-[14px] sm:text-[15px] leading-[1.8] sm:leading-[1.9] text-ink-muted mb-4">
            合理的重试策略能应对临时故障，但过度重试会加剧系统负担。
          </p>
...
# 5. 监控重试率，过高说明系统存在问题`}
            language="python"
            highlights={[6, 18, 35, 48, 60, 72, 84]}
            filename="retry-strategy.py"
            description="重试策略实现"
          />

          <Callout type="info" title="指数退避的优势">
            指数退避（Exponential Backoff）通过逐渐增加重试间隔，给故障系统恢复的时间，避免大量客户端同时重试导致的<strong>雪崩效应</strong>。添加随机抖动（Jitter）进一步分散重试时间，是分布式系统重试的最佳实践。
          </Callout>
          ...
          <p className="text-[14px] sm:text-[15px] leading-[1.8] sm:leading-[1.9] text-ink-muted mb-4">
            Exactly-Once 保证每条消息恰好被处理一次，是消息可靠性的最高境界。
          </p>

          <DiagramBlock title="Exactly-Once 实现方案">
            <pre className="font-mono text-[11px] sm:text-[12px] text-ink-muted leading-[1.6] whitespace-pre">{`
方案 1: 幂等生产者 + 事务消费者
┌──────────┐         ┌──────────┐         ┌──────────┐
...
# 处理逻辑
result = stream.filter(lambda k, v: v > 100) \
...
# - 事务会增加延迟（约 20-30%）
# - 事务 ID 必须全局唯一，重启后使用相同 ID 可恢复`}
            language="python"
            highlights={[6, 18, 31, 44, 57, 70]}
            filename="exactly-once.py"
            description="Exactly-Once 实现"
          />

          <SideNote label="Exactly-Once 的真相">
            业界常说的"Exactly-Once"通常是<strong>端到端幂等性</strong>，而非严格的精确一次。即使 Kafka 保证了消息不重复，如果消费者写入数据库时未做幂等，仍会重复。因此，<strong>应用层幂等是最后一道防线</strong>，不可或缺。
          </SideNote>
...
# - 测试环境：异步刷盘 + 1 副本 + acks=0`}
            language="python"
            highlights={[6, 17, 24, 33, 42, 52, 62, 72]}
            filename="persistence.py"
            description="持久化与副本配置"
          />

          <Callout type="warning" title="同步刷盘的性能影响">
            同步刷盘（SYNC_FLUSH）每次写入都要等待磁盘确认，吞吐量可能下降 50-80%。仅在<strong>金融、支付</strong>等对数据可靠性要求极高的场景使用。大多数场景使用异步刷盘 + 多副本已足够可靠。
          </Callout>
...
# - 定期演练：测试告警是否有效`}
            language="python"
            highlights={[6, 13, 20, 27, 34, 41, 48, 55, 62, 69, 82, 95]}
            filename="monitoring.py"
            description="监控与告警配置"
          />

          <SideNote label="监控工具推荐">
            <strong>Kafka</strong>：Kafka Exporter + Prometheus + Grafana<br/>
            <strong>RocketMQ</strong>：RocketMQ Dashboard + Prometheus<br/>
            <strong>RabbitMQ</strong>：内置 Management Plugin + Prometheus<br/>
            <strong>通用</strong>：Datadog、New Relic、Dynatrace（商业方案）
          </SideNote>
          ...
          <Callout type="danger" title="误区 1：使用了消息队列就保证可靠性">
            <p className="mb-2"><strong>错误认知</strong>：认为只要引入消息队列，消息就不会丢失。</p>
            <p><strong>正确理解</strong>：消息队列只是提供了可靠性<strong>机制</strong>，但需要正确配置和使用。如果生产者使用 acks=0、消费者使用自动 ACK、Broker 单副本，消息仍然可能丢失。可靠性是<strong>系统性工程</strong>，需要生产者、Broker、消费者三方配合。</p>
          </Callout>

          <Callout type="danger" title="误区 2：Exactly-Once 不需要幂等">
            <p className="mb-2"><strong>错误认知</strong>：认为 MQ 提供了 Exactly-Once 语义，消费者就不需要实现幂等。</p>
            <p><strong>正确理解</strong>：Kafka 的 Exactly-Once 仅限于 Kafka 生态内部。如果消费者将数据写入数据库或调用外部 API，仍可能因重试导致重复。<strong>应用层幂等是最后一道防线</strong>，无论 MQ 是否保证 Exactly-Once，消费者都应该是幂等的。</p>
          </Callout>

          <Callout type="danger" title="误区 3：重试次数越多越好">
            <p className="mb-2"><strong>错误认知</strong>：认为应该设置很大的重试次数，确保消息最终成功。</p>
            <p><strong>正确理解</strong>：过度重试会<strong>浪费资源</strong>并<strong>掩盖真正的问题</strong>。如果消息连续失败 10 次仍未成功，很可能是代码 Bug 或数据异常，而非临时故障。应设置合理的最大重试次数（如 3-5 次），之后转入死信队列人工处理。</p>
          </Callout>

          <Callout type="warning" title="误区 4：死信队列可以忽略">
            <p className="mb-2"><strong>错误认知</strong>：认为死信队列中的消息不重要，可以不处理。</p>
            <p><strong>正确理解</strong>：死信队列是<strong>系统健康的晴雨表</strong>。大量死信意味着存在严重问题：代码 Bug、数据异常、依赖服务故障。应建立死信监控和及时处理机制，定期分析死信原因并优化系统。忽略死信会导致数据不一致和业务损失。</p>
          </Callout>
          ...
          <InterviewSection questions={[
            {
              question: "如何保证消息不丢失？",
              answer: "三个层面：① 生产者：使用 acks=all（Kafka）或同步发送（RocketMQ），启用重试机制；② Broker：持久化（同步/异步刷盘），多副本（≥3），配置 min.insync.replicas；③ 消费者：手动 ACK，处理成功后再确认，实现幂等消费。端到端可靠性需要三者配合，任何环节薄弱都会导致消息丢失。"
            },
            {
              question: "如何实现幂等消费？",
              answer: "四种方案：① 唯一键约束：利用数据库唯一索引防止重复插入；② Redis 去重：记录已处理的消息 ID；③ 状态机：基于状态流转保证幂等；④ Token 机制：生产者生成唯一 Token，消费者验证。最常用的是唯一键约束，简单可靠。关键是确保同一消息多次处理结果一致。"
            },
            {
              question: "RocketMQ 事务消息的原理是什么？",
              answer: "通过半消息机制：① 生产者发送半消息到 Broker（对消费者不可见）；② 执行本地事务（如数据库操作）；③ 根据事务结果向 Broker 发送 Commit 或 Rollback；④ 如果生产者宕机，Broker 定期回查生产者事务状态；⑤ 根据回查结果决定提交或回滚半消息。这保证了消息发送和本地事务的最终一致性。"
            },
            {
              question: "什么是死信队列？什么时候使用？",
              answer: "死信队列（DLQ）存储多次重试仍失败的消息。使用场景：① 消息格式错误，无法解析；② 业务逻辑异常，如数据校验失败；③ 依赖服务长期不可用；④ 达到最大重试次数。死信队列避免失败消息阻塞正常消息，便于人工介入处理。应监控死信数量并定期处理。"
            },
            {
              question: "Kafka 的 Exactly-Once 是如何实现的？",
              answer: "通过幂等生产者和事务：① 幂等生产者：每个生产者分配唯一的 Producer ID，每条消息附带 Sequence Number，Broker 据此去重；② 事务：使用 init_transactions()、begin_transaction()、commit_transaction() 保证跨分区的原子性；③ 消费者使用 isolation_level=read_committed 只读取已提交的事务消息。但仅限于 Kafka 生态内部，跨系统仍需应用层幂等。"
            },
            {
              question: "如何处理消息积压？",
            ...
            {
              question: "ACK 机制有哪些类型？如何选择？",
              answer: "生产者 ACK：acks=0（最快，可能丢失）、acks=1（Leader 确认）、acks=all（所有 ISR 确认，最可靠）。消费者 ACK：自动 ACK（不推荐，可能丢失）、手动 ACK（推荐，处理成功后确认）。Broker ACK：同步复制（可靠，性能低）、异步复制（性能好，可能丢失）。关键业务使用 acks=all + 手动 ACK + 同步复制，非关键业务可适当放宽。"
            }
          ]} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-5">
            ...
            <a href="/docs/infra/messaging/kafka/kafka-reliability" className="block p-4 bg-parchment-light border border-border rounded-paper-md hover:border-accent hover:shadow-paper transition-all group">
              <div className="text-[11px] font-mono text-accent mb-1">深入学习 →</div>
              <div className="font-semibold text-ink group-hover:text-accent">Kafka 消息可靠性保证</div>
              <div className="text-[12px] text-ink-muted mt-1">ACK、ISR、副本同步</div>
            </a>
            <a href="/docs/infra/messaging/rocketmq/rocketmq-transaction" className="block p-4 bg-parchment-light border border-border rounded-paper-md hover:border-accent hover:shadow-paper transition-all group">
              <div className="text-[11px] font-mono text-indigo mb-1">核心技术 →</div>
              <div className="font-semibold text-ink group-hover:text-indigo">RocketMQ 事务消息</div>
            ...
            <a href="/docs/08-microservices/distributed-transaction" className="block p-4 bg-parchment-light border border-border rounded-paper-md hover:border-accent hover:shadow-paper transition-all group">
              ...
              <div className="text-[12px] text-ink-muted mt-1">2PC、TCC、Saga、最终一致性</div>
            </a>
          </div>

          <Callout type="info" title="学习建议">
            消息可靠性是分布式系统的核心课题，建议通过以下方式深入学习：① 官方文档：阅读各 MQ 的可靠性配置指南；② 源码分析：研究 ACK、副本同步、事务实现；③ 故障演练：模拟节点宕机、网络分区，观察系统行为；④ 性能测试：压测不同配置下的吞吐量和延迟；⑤ 生产实践：参与实际项目的 MQ 运维和优化。理论结合实践才能真正掌握可靠性保障技术。
          </Callout>
